...
def tackPlots(tackAnalysed,windowSize,avgTack,tackPlotDict,colour,name):
    ...
    if avgTack.size == 0: # check for condition here when there are zero tacks and avgTack has size 0
        ...
        LimVMG=max(avgTack[:,2])*1.1 # known bug here when there are zero tacks and avgTack has shape (0,6)
...
def gybePlots(gybeAnalysed,windowSize,avgGybe,gybePlotDict,colour,name):
    ...
    if avgGybe.size == 0: # check for condition here when there are zero gybes and avgGybe has size 0
        # make empty figure
