from GUI import fileSelectionWindow, passToTk, saveGraph, versionChecker, setCrops, updateCropSliders, browseFiles
from tackAnalysis import analyseManoeuvresCubicInterp, crop
from straightLineAnalysis import straightLineAnalysisCubic, polarPlotsCubic
from mapPlots import plotmapsCubic
...
def MainUpdateGraphs(filenameList,windAngleList,satelliteBool,crops):
    global tackPlotDict, gybePlotDict, violinPlotDict, mapPlotDict, analysedDataDict, straightLineDataDict, polarPlotDict
    ...
    polarPlotDict,polarDataDict = polarPlotsCubic(filenameList,straightLineDataDict,browseData.colours)
    polarCanvas= FigureCanvasTkAgg(polarPlotDict['fig'], master=app_window.children["!notebook"].children["!frame6"]) 
    polarCanvas.get_tk_widget().pack(side='top', fill='both', expand=True)
...
app_window.children["!notebook"].children["!frame6"].children["!button"].configure(command=lambda:saveGraph(polarPlotDict['fig']))
...
# Update browseFiles definition to include app_window
app_window.children["!notebook"].children["!frame"].children["!frame"].children["!button"].configure(command=lambda:browseFiles(app_window,1,app_window.children["!notebook"].children["!frame"].children["!frame"].children["!frame"]))
app_window.children["!notebook"].children["!frame"].children["!frame"].children["!button2"].configure(command=lambda:browseFiles(app_window,2,app_window.children["!notebook"].children["!frame"].children["!frame"].children["!frame"]))
